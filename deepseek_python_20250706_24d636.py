...
def run_fraud_analysis(df):
    ...
    # Convert RegistrationDate to datetime with error handling
...
def analysis_page():
    ...
    # Use placeholder index 1 (subtitle placeholder)
    ...
    slide = prs.slides.add_slide(slide_layout)
    title = slide.shapes.title
    title.text = "Fraud Logic Flags Distribution"
    
    # Create and save chart
    fig, ax = plt.subplots(figsize=(10, 6))
    flag_counts = df[fraud_logic_cols].sum().sort_values(ascending=False)
    flag_counts.plot(kind='bar', ax=ax, color='#4f46e5')
    ax.set_ylabel('Flag Count')
    ax.set_title('Fraud Flag Distribution', fontsize=14)
    plt.tight_layout()
    ...
    left = Inches(0.5)
    top = Inches(1.5)
    slide.shapes.add_picture(buf, left, top, width=Inches(9))
