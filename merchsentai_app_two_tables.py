...
def analysis_page():
    ...
    if st.session_state.processor_done:
        ...
        # Table 1: raw cost incurred
        incurred = pd.DataFrame([
            {'Issue':'Sanction Hits','Count':rm['Sanction Hits'],'Cost ($)':rm['Sanction Cost']},
            {'Issue':'Suspicious Incidents','Count':rm['Suspicious Incidents'],'Cost ($)':rm['Fraud Cost']},
            {'Issue':'Total Loss','','Cost ($)':rm['Total Loss']}
        ])
        st.markdown("**Cost Incurred (Raw Data Processor)**")
        st.table(incurred)
        # Table 2: cost saved due to MerchSentAI
        saved = pd.DataFrame([
