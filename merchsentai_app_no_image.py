...
def simulate_cost(df, fraud_cost, sanction_cost):
    ...
    return {
        'Sanction Hits': sanction_misses,
        'Sanction Cost': sanction_total,
        'Suspicious Incidents': fraud_misses,
        'Fraud Cost': fraud_total,
        'Total Loss': total
    }
...
def analysis_page():
    ...
    if st.session_state.screened:
        checks = {'Sanction Hits': int(df['SanctionedFlag'].sum())}
    ...
    if st.session_state.fraud_done:
        summary = {
            'Total Merchants': len(df),
            'Suspicious Merchants': int(df['IsSuspicious'].sum())
        }
        ...
        with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
            pd.DataFrame([summary]).to_excel(writer, sheet_name='FraudSummary', index=False)
            susp.to_excel(writer, sheet_name='SuspiciousMerchants', index=False)
        buf.seek(0)
        st.download_button("Download Fraud Summary (Excel)", buf.getvalue(),
                           file_name="fraud_summary.xlsx",
                           mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    # Stage 4: Processor Impact
    ...
    if st.session_state.processor_done:
        ...
        impact = pd.DataFrame([{
            'Issue': 'Sanction Hits',
            'Raw Count': rm['Sanction Hits'],
            'Raw Cost ($)': rm['Sanction Cost'],
            'Refined Count': ref['Sanction Hits'],
            'Refined Cost ($)': ref['Sanction Cost'],
            'Savings ($)': rm['Sanction Cost'] - ref['Sanction Cost']
        },{
            'Issue': 'Suspicious Incidents',
            'Raw Count': rm['Suspicious Incidents'],
            'Raw Cost ($)': rm['Fraud Cost'],
            'Refined Count': ref['Suspicious Incidents'],
            'Refined Cost ($)': ref['Fraud Cost'],
            'Savings ($)': rm['Fraud Cost'] - ref['Fraud Cost']
        },{
            'Issue': 'Total Loss',
            'Raw Count': '',
            'Raw Cost ($)': rm['Total Loss'],
            'Refined Count': '',
            'Refined Cost ($)': ref['Total Loss'],
            'Savings ($)': rm['Total Loss'] - ref['Total Loss']
        }])
        st.table(impact)
        total_savings = rm['Total Loss'] - ref['Total Loss']
        st.markdown(f"**MerchSentAI Impact Summary:** By enriching and screening, the payment processor avoided **${total_savings:,}** in losses.")
